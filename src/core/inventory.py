...
# Master Database structured logically by equipment slots inspired by the Ultima Wiki.
# Mapping structure: Slot Name -> List of allowed game objects with rich metadata.
# The 'id' field matches the exact C# EObjectType Enum values for engine serialization.
WIKI_ITEM_DATABASE = {
    "Light Source": [
    ...
    "Right Hand": [
        # --- Edge Slashing/Thrusting Weapons ---
...
def get_item_from_any_slot(object_type_id: int) -> dict:
    """
    Scans the entire database to find a fallback description if an item 
    is found in an unexpected or custom inventory slot setup.
    """
    for slot, items in WIKI_ITEM_DATABASE.items():
        for item in items:
...
def get_equipment_summary(data: dict) -> list:
    """
    Extracts equipped items from save data and maps them logically to the 
    Wiki-defined layout. Completely removes raw sprite index reliance.
    """
    slot_names = [
    ...
    if not data or not raw_equipment:
        logger.warning("Key 'equippedItems' missing inside the save file.")
        return [{
            "slot_index": idx, "slot_name": name, "objectName": "Empty Slot",
            "objectTypeName": "", "objectType": 0, "objectIndex": 0, "is_empty": True
    ...
    for idx, slot_title in enumerate(slot_names):
        if idx < len(raw_equipment):
            ...
            if obj_type == 0:
                item_details = {"name": "Empty Slot", "details": "Nothing equipped"}
            else:
                # Look inside its dedicated slot list first
                allowed_items = WIKI_ITEM_DATABASE.get(slot_title, [])
                match = next((i for i in allowed_items if i["id"] == obj_type), None)
                
                # Global fallback scanning all slots
                if not match:
                ...
                if match:
                    item_details = {
                        "name": match["name"],
                        "details": f"{match.get('details', '')} (Wgt: {match.get('weight', '0')})"
                    }
                else:
                    item_details = {
                        "name": item_data.get("objectName", f"Unknown #{obj_type}"),
                        "details": "Custom loaded entity"
                    }
            
            equipment_summary.append({
                "slot_index": idx,
                "slot_name": slot_title,
                "objectName": item_details["name"],
                "objectType": obj_type,
                "objectIndex": item_data.get("objectIndex", 0),
                "details": item_details.get("details", ""),
                "is_empty": obj_type == 0
            })
            
    return equipment_summary

def update_equipped_item(data: dict, slot_index: int, new_object_type: int, new_name: str):
    """
    Mutates character equipment payload values safely within memory.
    """
    raw_equipment = find_key_recursive(data, "equippedItems")
    if not raw_equipment or slot_index >= len(raw_equipment):
        logger.error("Out of bounds or invalid equipment structure mutation attempt.")
        return
    ...
    # Generic logic category assignment based on standard items
    if int(new_object_type) == 0:
        item_slot["objectName"] = ""
        item_slot["objectTypeName"] = ""
        item_slot["objectIndex"] = 0
    else:
        # Resolve item generic categorization for engine safety
        match = get_item_from_any_slot(new_object_type)
        if match:
            item_slot["objectTypeName"] = "Weapon" if "damage" in match else "Armour"
            
    logger.info("Slot #%d written with Type %d (%s).", slot_index, new_object_type, new_name)